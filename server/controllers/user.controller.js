...
class UserController {
   ...
   async checkAuthUser(req, res) {
      try {
         ...
         res.status(200).json({ message: responseMessages.success, data: username });
      } catch (error) {
   ...
   async regUser(req, res) {
      ...
      try {
         ...
         await db.query(`
            INSERT INTO persons (username, name, surname, birth, city, avatar, password, token)
   ...
   //! нужно сделать фукнционал на фронте/беке
   async updateUser(req, res) {
      ...
      try {
         ...
         if (img.length) {
            const currentImg = await db.query(`
               SELECT img
               FROM person
               WHERE id = $1`,
            [user_id]);

            await cloudinary.uploader.destroy(currentImg.rows[0].img);
